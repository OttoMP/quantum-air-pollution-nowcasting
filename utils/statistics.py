...
def get_mean_left_right_error_interval(model, X_val, y_val, y_test, y_pred):
    error, error_left, error_right = get_error_interval(model, X_val, y_val, y_pred, 0.95)

    mean_error      = np.ndarray((1,y_test.shape[1]))
    mean_error_left = np.ndarray((1,y_test.shape[1]))
    mean_error_right= np.ndarray((1,y_test.shape[1]))
    mean_predictions       = np.ndarray((1,y_pred.shape[1]))

    for ii in range(y_test.shape[1]):
        mean_error_left[0,ii]  = np.mean(error_left[:,ii])
        mean_error_right[0,ii] = np.mean(error_right[:,ii])
        mean_predictions[0,ii]        = np.mean(y_pred[:,ii])

    mean_error = (mean_error_right - mean_error_left)/2

    return mean_predictions, mean_error, mean_error_left, mean_error_right
...
def quantitative_analysis(y_test, y_test_pred):
    ...
    for i in range(y_test.shape[1]):
        ...
        valores.append([str(i+1)+" hours",mae,mse,nmse,rmse,nrmse,r_value,rr,fat])
        print(f"\n{i+1} hours ahead")
        print("MAE:",mae)
        print("MSE:",mse)
        print("NMSE:",nmse)
